...
@synthesize consumerSecret = __consumerSecret, consumerKey = __consumerKey;
...
- (BOOL) isAuthorized {	
	if (__accessToken.key && __accessToken.secret) return YES;
	...
	if (accessTokenString.length) {				
		[__accessToken release];
		__accessToken = [[OAToken alloc] initWithHTTPResponseBody: accessTokenString];
		[self setUsername: [self extractUsernameFromHTTPBody: accessTokenString] password: nil];
		if (__accessToken.key && __accessToken.secret) return YES;
	}
	
	[__accessToken release];										// no access token found.  create a new empty one
	__accessToken = [[OAToken alloc] initWithKey: nil secret: nil];
	return NO;
}
...
- (void) setAccessToken: (OAServiceTicket *) ticket withData: (NSData *) data {
	...
	[__accessToken release];
	__accessToken = [[OAToken alloc] initWithHTTPResponseBody:dataString];
}
